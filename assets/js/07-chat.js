...
async function sendMessage() {
  ...
  if (/^\s*\/gallery-download\b/i.test(text)) {
    try {
      const selection = JSON.parse(
        text.replace(/^\s*\/gallery-download\b\s*/i, ""),
      );
    ...
    try {
      const selection = JSON.parse(
        text.replace(/^\s*\/mediadownload\b\s*/i, ""),
      );
      const count = Array.isArray(selection.selectedIndices)
        ? selection.selectedIndices.length
        : 0;
  ...
  runSession.history.push(buildTurnUserMessage(messageToSend, historyImages));
  ...
  try {
    ...
    if (runMode === "ollama") {
        ...
        (evt) => {
          ...
          if (evt.type === "skill_links") {
            if (
              ...
              runSession.streamingAssistantDiv = addMessage("", "assistant");
            }
      ...
      setDraftAssistant(runMode, response || "", activeLibrarySources.results);
      if (
        thinking?.isConnected &&
        !thinking.hadReasoning &&
        !thinking.hadTrace &&
      ...
      response = await runPiRpcConversation(
        ...
        (partialResponse) => {
          ...
          setDraftAssistant(runMode, nextText, activeLibrarySources.results);
        },
        (evt) => {
          captureLibrarySources(
            evt,
    ...
    if (e.name === "AbortError" || runAbortController.signal.aborted) {
      ...
      const cancelled =
        prior && prior.trim()
          ...
          : "*Request cancelled by user.*";
      setDraftAssistant(runMode, cancelled, activeLibrarySources.results, {
...
async function regenerate(wrapEl) {
  ...
  let hardModeOverride = null;
  if (runMode === "ollama") {
    const context = buildOllamaRequestContext(requestHistory, lastUserMessage);
    requestHistory = context.requestHistory;
...
function scheduleQueueDrain(modeName) {
  setTimeout(() => {
    ...
    session.queue = session.queue.filter((q) => q.convId === currentConvId);
...
function organizeSettingsTabs() {
  ...
  if (settingsBody && settingsFooter) settingsBody.appendChild(settingsFooter);
}
...
function updateSettingsTabAvailability(state = {}) {
  ...
  const isCloudMode = typeof state === "object" && state.isCloudMode === true;
  const isLocalMode = typeof state === "object" && state.isLocalMode === true;
...
document.addEventListener("mousemove", (e) => {
  ...
  if (isResizing) {
    ...
    const newWidth = e.clientX - historyPanelEl.getBoundingClientRect().left;
    if (newWidth > 150 && newWidth < mainRect.width - 200) {
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  if (
    ...
    localStorage.setItem(activePromptStorageKey("ollama"), legacyActivePrompt);
  }
  ...
  await __timed("refreshOllamaPromptOverrides", refreshOllamaPromptOverrides);
  ...
  const btn_saveBookSearch = document.getElementById("saveBookSearchConfigBtn");
...
function syncCustomSelect(selectEl) {
  ...
  if (
    ...
    selectEl.nextElementSibling.classList.contains("custom-select-wrapper")
  ) {
  ...
  if (!selectEl.dataset.observerAttached) {
    const observer = new MutationObserver(() => {
      if (
        ...
        selectEl.nextElementSibling.style.display = selectEl.classList.contains(
          "mode-hidden",
        )
          ? "none"
          : "inline-block";
...
document.addEventListener("DOMContentLoaded", function () {
  document.addEventListener("change", function (e) {
    if (e.target.classList.contains("builtin-skill-toggle")) {
      ...
      if (e.target.id === "cloudModelInput") {
        ...
        cloudSettings.maxTokens = clampInteger(e.target.value, 2048, 1, 128000);
      }
      updateModeStatus();
    } else if (
  ...
  document.addEventListener("click", function (e) {
    const removeSourceButton = e.target.closest(".database-source-remove");
  ...
  const btn_closeSettingsBtn = document.getElementById("closeSettingsBtn");
  ...
  if (btn_resetSearchAlgorithmSettingsBtn) {
    btn_resetSearchAlgorithmSettingsBtn.addEventListener("click", function () {
  ...
  const bookFilterCloseBtn = document.getElementById("bookFilterCloseBtn");
  if (bookFilterCloseBtn) {
    bookFilterCloseBtn.addEventListener("click", closeBookFilterPanel);
  }
  const bookFilterOverlayEl = document.getElementById("bookFilterOverlay");
  ...
  const bookFilterClearBtn = document.getElementById("bookFilterClearBtn");
  ...
  const bookFilterApplyBtn = document.getElementById("bookFilterApplyBtn");
  ...
  if (librarySearchEnabledInputEl) {
    librarySearchEnabledInputEl.addEventListener("change", updateBookFilterUi);
  }
  ...
  if (btn_copyIndexedFilesExportPathBtn) {
    btn_copyIndexedFilesExportPathBtn.addEventListener("click", function () {
  ...
  const btn_runLibraryIndexBtn = document.getElementById("runLibraryIndexBtn");
  ...
  if (btn_forceLibraryReindexBtn) {
    btn_forceLibraryReindexBtn.addEventListener("click", async function () {
      if (
        !(await appConfirm("Reindex all configured source files?", "Database", {
  ...
  const btn_refactored_btn_1 = document.getElementById("refactored_btn_1");
  ...
  const btn_refactored_btn_2 = document.getElementById("refactored_btn_2");
  ...
  const btn_refactored_btn_3 = document.getElementById("refactored_btn_3");
  ...
  const btn_refactored_btn_4 = document.getElementById("refactored_btn_4");
  ...
  const btn_refactored_btn_5 = document.getElementById("refactored_btn_5");
  ...
  const btn_refactored_btn_6 = document.getElementById("refactored_btn_6");
  ...
  const btn_refactored_btn_7 = document.getElementById("refactored_btn_7");
  ...
  const btn_refactored_btn_8 = document.getElementById("refactored_btn_8");
  ...
  const btn_refactored_btn_9 = document.getElementById("refactored_btn_9");
  ...
  const btn_refactored_btn_10 = document.getElementById("refactored_btn_10");
  ...
  const btn_refactored_btn_11 = document.getElementById("refactored_btn_11");
  ...
  const btn_refactored_btn_12 = document.getElementById("refactored_btn_12");
  ...
  const btn_refactored_btn_13 = document.getElementById("refactored_btn_13");
