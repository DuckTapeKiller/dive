...
const databaseSettingsGroup = document.getElementById("databaseSettingsGroup");
const piFontGroup = document.getElementById("piFontGroup");
const cloudFontGroup = document.getElementById("cloudFontGroup");
const promptSettingsGroup = document.getElementById("promptSettingsGroup");
...
const API_BASE =
  window.location.protocol === "moz-extension:" ? "http://127.0.0.1:8080" : "";
...
async function readJsonResponse(res, contextLabel) {
  ...
  if (!res.ok) {
    const details =
      raw && raw.trim() ? raw.trim().slice(0, 500) : "empty response body";
  ...
  try {
    ...
    throw new Error(`${contextLabel} returned invalid JSON: ${e.message}`);
  }
}
...
function persistEnabledModes() {
  localStorage.setItem(ENABLED_MODES_STORAGE_KEY, JSON.stringify(enabledModes));
  saveUiSettingsSoon();
}
...
function renderDefaultModeList() {
  ...
  const options = [
    ...
    ...MODE_DEFS.filter((def) => enabledModes.includes(def.id)).map((def) => ({
      id: def.id,
      label: def.label,
    })),
  ];
...
async function loadUiSettings() {
  ...
  piFont = useServer
    ...
    : normalizeFontStack(
        localStorage.getItem("ollama-pi-chat-pi-font") || DEFAULT_UI_FONTS.pi,
      );
  cloudFont = useServer
    ? legacyFontsUntouched
  ...
  for (const id of Object.keys(thinkingExpandedByMode)) {
    const raw =
      useServer && storedThinkingExpanded[id] !== undefined
        ...
        : localStorage.getItem(`ollama-pi-chat-${id}-thinking-expanded`);
    if (raw !== null && raw !== undefined && raw !== "") {
