    ...
    marked.use({
      renderer: {
        code(tokenOrCode, maybeLang) {
          const text =
            typeof tokenOrCode === "object" ? tokenOrCode.text : tokenOrCode;
          const lang =
            typeof tokenOrCode === "object" ? tokenOrCode.lang : maybeLang;
          const language = (lang || "").match(/\S*/)[0] || "";
          const highlighted = highlight(text, language);
          const cls = language ? `hljs language-${language}` : "hljs";
          const inner = highlighted != null ? highlighted : escapeHtml(text);
...
function __recordError(msg) {
  const clean = String(msg)
    ...
    .trim()
    .slice(0, 240);
  __capturedErrors.push(new Date().toISOString().slice(11, 19) + "  " + clean);
...
(function () {
  ...
  console.error = function (...args) {
    __recordError(args.map((a) => (a?.message ? a.message : a)).join(" "));
...
function normalizeSearchAlgorithmValues(raw, fallback) {
  return {
    ...
    keywordWeight: clampFloat(raw?.keywordWeight, fallback.keywordWeight, 0, 3),
    ...
    sourceWeight: clampFloat(raw?.sourceWeight, fallback.sourceWeight, 0, 3),
...
let ollamaAgentMode = localStorage.getItem(OLLAMA_AGENT_MODE_KEY) === "true";
...
let ollamaAgentMaxRounds = (() => {
  const v = parseInt(localStorage.getItem(OLLAMA_AGENT_ROUNDS_KEY) || "25", 10);
...
function wireOllamaAgentSettings() {
  ...
  if (cb) {
    ...
    cb.addEventListener("change", () => {
      ...
      localStorage.setItem(OLLAMA_AGENT_MODE_KEY, String(ollamaAgentMode));
    });
  }
  if (rounds) {
  ...
  if (nativeCb) {
    ...
    nativeCb.addEventListener("change", () => {
      ...
      localStorage.setItem(OLLAMA_NATIVE_TOOLS_KEY, String(ollamaNativeTools));
    });
  }
...
const ALL_BUILTIN_SKILLS_INFO = {
  ...
  code_search: {
    ...
    example:
      '{"action": "grep", "path": "~/dive/workspace", "pattern": "TODO"}',
  },
  git_tools: {
  ...
  task_plan: {
    ...
    example:
      '{"action": "create", "steps": ["Search sources", "Write summary"]}',
  },
};
...
function humanizeSkillLabel(name) {
  ...
  return words
    .map((word, index) => {
      ...
      if (Object.prototype.hasOwnProperty.call(SKILL_LABEL_WORDS, lower)) {
