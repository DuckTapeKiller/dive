...
Tools are functions the model can call during a turn. Dive ships 29, defined in
[`skills.js`](../skills.js) as `ALL_SKILLS` and implemented across `skills/`.
File names, identifiers and API paths keep the older name "skill"; the interface
and these documents call them tools.

Tools are available in every mode with `diveSkills: true` — that is, every mode
except Pi, which brings its own tools.

Settings lists these as Native tools. External tools (plugins), custom tools
and MCP servers add more. Skills are a different thing: folders of instructions
the model reads, documented in [skills.md](skills.md).
When at least one skill is enabled, the model also gets the `activate_skill`
tool described there.
...
A real Chromium the model drives, shown in a panel beside the chat.

| Tool          | Arguments                                      | Notes                                                     |
| ------------- | ---------------------------------------------- | --------------------------------------------------------- |
| `browse_read` | **action**, url, selector, session, show       | Open, read, list elements, screenshot, back, close        |
| `browse_act`  | **action**, target, text, key, amount, session | Click, type, press, select, scroll; requires confirmation |
...
**Arguments that are not valid JSON are refused.** The tool does not run. The
model receives an error naming the tool so it can correct the call. Running with
`{}` would be a guess: the tool either fails somewhere unrelated, or succeeds on
...
it to English precisely because of that. Tools that care supply their own
...
Empty or absent arguments are fine — that is how a no-argument tool like
`time_and_date` gets called.

## Enabling and disabling

Per mode, in Settings > Tools, saved through
`/api/ollama/skills/settings?mode=<mode>`. A disabled tool returns an error if
called rather than executing.
...
The largest tool by far, and not simply a bigger web search. It runs an evidence
pipeline:
...
Two routes: custom tools in Settings > Tools (stored through
`/api/custom-skills`), or a plugin. See [plugins.md](plugins.md). MCP servers
add tools too; see [mcp.md](mcp.md). For instructions rather than code, write a
skill: see [skills.md](skills.md).
